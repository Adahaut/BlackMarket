...
{
    badge_Properties* props = (badge_Properties*)properties;

    float px, py; // X and Y of the player
    static int x = 100, y = 70; // Spawn X and Y of the object
...
{
    badge_Properties* props = (badge_Properties*)H3_Object_GetComponent(object, BADGE_TYPEID)->properties;
    if (collide.other)
        props->a = false;

}

void BADGE_NoCollide(H3Handle object, H3Handle other)
{
    badge_Properties* props = (badge_Properties*)H3_Object_GetComponent(object, BADGE_TYPEID)->properties;
    if (other == NULL)
        props->a = true;
...
void* BADGE_CreateProperties(H3Handle player, H3Handle texte)
{
    badge_Properties* properties = malloc(sizeof(badge_Properties));
...
H3_DEFINE_COMPONENT_PROPERTY_ACCESSORS_RW_EX(badge, BADGE_TYPEID, int, slots);