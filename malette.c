...
{
    malette_Properties* props = (malette_Properties*)properties;

    float px, py; // X and Y of the player
    static int x = 2470, y = 2380; // Spawn X and Y of the object
...
{
    malette_Properties* props = (malette_Properties*)H3_Object_GetComponent(object, MALETTE_TYPEID)->properties;
    if (collide.other)
        props->a = false;

}

void MALETTE_NoCollide(H3Handle object, H3Handle other)
{
    malette_Properties* props = (malette_Properties*)H3_Object_GetComponent(object, MALETTE_TYPEID)->properties;
    if (other == NULL)
        props->a = true;
...
{
    malette_Properties* properties = malloc(sizeof(malette_Properties));
...
H3_DEFINE_COMPONENT_PROPERTY_ACCESSORS_RW_EX(malette, MALETTE_TYPEID, int, slots);